#tag Class
Protected Class BindableWebPopupMenu
Inherits WebPopupMenu
Implements ModelBinding.IBindableWebControl
	#tag Event
		Sub SelectionChanged(item As WebMenuItem)
		  Me.UpdateBindValue(Me.BindProperty, item.Text)
		  RaiseEvent SelectionChanged(item)
		End Sub
	#tag EndEvent
	...
	#tag Method, Flags = &h0
		Sub ApplyBindValue(value as Variant)
		  ...
		  If value.IsNumeric Then
		    Me.SelectedRowIndex = value.IntegerValue - 1
		  Else
		    If Me.HasValue(value) Then
		      Me.SelectRowWithText(value)
		    End
		  End
		End Sub
	#tag EndMethod

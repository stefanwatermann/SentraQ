...
Protected Class DocumentationService
	#tag Method, Flags = &h0
		Function CreateHtmlDocu(station as StationModel) As string
		  ...
		  .Replace("{Longitude}", Str(station.Longitude))_
		  .Replace("{AlertMailReceiver}", station.AlertReceiverEmailAddresses)_
		  .Replace("{Maintenance}", If(station.MaintenanceActive, "aktiviert am " + station.MaintenanceActiveSinceTs.AsLongDateTimeString, "nicht aktiv"))
